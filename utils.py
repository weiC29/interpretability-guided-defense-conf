...
def get_loaders(dataset, dir_, batch_size, arch, mode='Crop288', num_workers=0):
    ...
    pin_mem = False
    if dataset == 'cifar10':
        train_eval_dataset = datasets.CIFAR10(dir_, train=True, transform=test_transform, download=True)
        test_dataset = datasets.CIFAR10(dir_, train=False, transform=test_transform, download=True)
    elif dataset == 'cifar100':
        train_eval_dataset = datasets.CIFAR100(dir_, train=True, transform=test_transform, download=True)
...
def evaluate_standard_classwise(test_loader, model, layer_name='layer4', num_classes=10, ablated_units=None):
    true_counts = torch.zeros(num_classes, dtype=torch.int64, device=DEVICE)
    pred_counts = torch.zeros(num_classes, dtype=torch.int64, device=DEVICE)
    correct_counts = torch.zeros(num_classes, dtype=torch.int64, device=DEVICE)
    total_count = 0
    model.eval()
    with torch.no_grad():
        for i, (X, y) in enumerate(test_loader):
            ...
            X = X.to(DEVICE)
            y = y.to(DEVICE)
            with autocast(device_type=AMP_DEVICE_TYPE, enabled=(AMP_DEVICE_TYPE != "cpu")):
    ...
    true_neg_counts = (total_count - true_counts) - (pred_counts - correct_counts)
    ...
    true_neg_rate = (true_neg_counts.float() / (total_count - true_counts).float()).cpu()
...
def save_predicted_logits(test_loader, model, ckpt_name, num_classes=10):
    output_logits = torch.zeros((len(test_loader.dataset), num_classes))
...
def compare_predicted_probs(test_loader, model, ckpt_name,
                            layer_name='layer4', ablated_units=None,
                            num_classes=10, use_probs=False, gt_only=False):
    ...
    difference_accumulator /= len(test_loader.dataset)
    if gt_only:
